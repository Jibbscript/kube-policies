...
| RBAC least-privilege (core services) | scoped to required verbs/resources | `templates/rbac.yaml`. Shared `ClusterRole` is cluster-wide **read-only** (`get,list,watch`) on core/apps/networking/RBAC resources, with **write** verbs scoped to the `policies.kube-policies.io` CRDs, `admissionregistration` webhook configs, `events`, and `coordination` Leases. No wildcard verbs/resources. | Partial (current) — *broad cluster-wide read; scope to namespaced informers + per-component SAs in P3 (least-privilege SA, ICX-06).* |
| Per-component service accounts | one SA per component | `AST-WH` and `AST-PM` **share** one SA (`kube-policies.serviceAccountName`); `AST-DB` has its own (`dashboard-rbac.yaml`). | Target (phase P3) — *split webhook/manager SAs.* |
