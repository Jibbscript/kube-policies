...
- New config files must validate against `internal/config.validateConfig` — particularly `policy.failure_mode` ∈ {`fail-open`, `fail-closed`}, `audit.backend` ∈ {`file`, `stdout`}, and `security.tls.min_version` = `1.3`.
